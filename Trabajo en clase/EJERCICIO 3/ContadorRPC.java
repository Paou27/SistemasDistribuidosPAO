...
/**
 * Invocación remota EN GRUPO: una llamada se ejecuta en TODOS los miembros
...
public class ContadorRPC implements Receiver {
    ...
    // ===== Métodos que se ejecutan REMOTAMENTE en cada miembro (públicos) =====
    public int incrementar(int cantidad) {
        int nuevo = contador.addAndGet(cantidad);
        System.out.println(" -> incrementar(" + cantidad + ") ejecutado aquí. Ahora vale " + nuevo);
        return nuevo;
    }
    ...
    private void invocarEnTodos(MethodCall llamada) throws Exception {
        // GET_ALL = espera la respuesta de TODOS los miembros, máximo 5 s
